...
The v2.3.D GCU LTS board includes a BMI270 6-axis IMU and a BMM150 3-axis magnetometer connected via I2C at 1 MHz.
...
IMU data is appended to the sensor frame payload when `IMU_FLAG` (0x01) is set in the packet header flags byte.
...
Appended after the IMU block when both `IMU_FLAG` (0x01) and `MAG_FLAG` (0x04) are set.
