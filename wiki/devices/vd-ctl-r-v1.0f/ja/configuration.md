...
|-----------|---------|---------|------|
| `mode` | `"off"` | `off`, `enabled` | LED ストリップの動作モード |
| `preset` | `"system_status"` | `system_status`, `connectivity`, `pressure_meter`, `stream_heartbeat`, `calibration_auto`, `solid_marker`, `identify`, `off` | ストリップ有効時に動かす表示プリセット |
| `color` | `"teal"` | `teal`, `green`, `blue`, `purple`, `amber`, `red`, `white` | `solid_marker` で使うマーカー色 |
| `brightness` | 0.35 | 0.0–1.0 | ストリップの輝度 |

完全なプリセット挙動は `led-status.md` を参照してください。ストリップは GPIO 12 に接続された 3 ピクセルの WS2812B です。
