...
| ビット | 値 | 名前 | 意味 |
|--------|-----|------|------|
| 0 | 0x01 | `IMU_FLAG` | ペイロードに BMI270 IMU データを含む（7 float） |
| 1 | 0x02 | `BATTERY_FLAG` | ペイロードにバッテリーデータを含む（4 バイト） |
| 2 | 0x04 | `MAG_FLAG` | ペイロードに BMM150 磁力計データを含む（3 float、IMU_FLAG が設定されている場合のみ。v1.0.F では送信されません） |
...
2. **BMI270 IMU データ** — 28 バイト、`IMU_FLAG` が設定されている場合のみ：
   - `acc[3]` — 加速度計 X、Y、Z（各 float32）
   - `gyro[3]` — ジャイロスコープ X、Y、Z（各 float32）
   - `temperature_c` — 温度（°C、float32）
3. **BMM150 磁力計データ** — 12 バイト、`IMU_FLAG` と `MAG_FLAG` の両方が設定されている場合のみ。v1.0.F には BMM150 が搭載されていないため、このブロックは通常出現しません：
