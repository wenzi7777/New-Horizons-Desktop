...
- ファームウェア v1.5.0 以降、`pixel` または `meter` を使うアプリが実行中の間は、アプリがプリセットより優先してストリップを制御し（`active_preset` は `app`）、アプリを停止するとプリセットに戻ります。`mode` と明るさは引き続き有効で、ストリップが `off` のときはアプリも何も表示しません。
