...
v2.1 GCU LTS ボードには、I2C 経由で接続された BMI270 6 軸 IMU（加速度計 + ジャイロスコープ）と BMM150 3 軸磁力計が搭載されています。
...
`IMU_FLAG`（0x01）と `MAG_FLAG`（0x04）の**両方**が設定されている場合、IMU ブロックの直後に追加されます。
...
`set_imu` コマンドで IMU を切り替えられます：
```json
{"command": "set_imu", "protocol": "NHO/Arduino/1", "enabled": true}
```

無効にすると、送信パケットに `IMU_FLAG` も `MAG_FLAG` も設定されません。
